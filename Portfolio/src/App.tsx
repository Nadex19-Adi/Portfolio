...
import { 
  SiHuggingface, SiScikitlearn, SiPython, SiMysql, SiGnubash, 
  SiCplusplus, SiTypescript, SiJavascript, SiReact, SiVite, 
  SiFastapi, SiStreamlit, SiNodedotjs, SiSupabase, SiFlask, SiDocker 
} from 'react-icons/si';
...
const EVENTS = [
  {
    name: "TechXHack 2026",
    tech: ["Participant", "AI Track"],
    desc: "Participated in a 48-hour hackathon focusing on GenAI applications.",
...
export default function App() {
  ...
  const [activeProjectTab, setActiveProjectTab] = useState<'group' | 'solo' | 'events'>('group');
          ...
          <main className="relative z-10 mx-auto w-full max-w-[1440px] px-6">
            ...
            <motion.section variants={container} initial="hidden" animate="show" id="home" className="relative min-h-screen flex items-center justify-center pt-24 pb-12">
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none overflow-hidden opacity-[0.03]" aria-hidden="true">
                <ScrollReveal baseOpacity={0} blurStrength={8} containerClassName="font-display text-[15vw] leading-none whitespace-nowrap">ENGINEERING</ScrollReveal>
                <ScrollReveal baseOpacity={0} blurStrength={8} containerClassName="mt-[-5vw] font-display text-[15vw] leading-none whitespace-nowrap">THE FUTURE</ScrollReveal>
              </div>
            ...
            {/* ABOUT SECTION */}
            <motion.section id="about" className="mt-[120px] space-y-12">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-serif text-[30px] md:text-[30px] leading-[1.1] font-medium tracking-tight text-text-primary ios-heading">About Me</h2>
              </div>
            ...
            <motion.section className="mt-[120px] space-y-8">
              <div className="flex items-center gap-3">
                ...
                <ScrollReveal baseOpacity={0} blurStrength={4} containerClassName="font-serif text-[30px] md:text-[30px] leading-[1.1] font-medium tracking-tight text-text-primary ios-heading">How I Work</ScrollReveal>
              </div>
            ...
            <motion.section id="experience" variants={item} className="mt-[120px]">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <ScrollReveal baseOpacity={0} blurStrength={4} containerClassName="font-serif text-[30px] md:text-[30px] leading-[1.1] font-medium tracking-tight text-text-primary ios-heading">Experience</ScrollReveal>
                </div>
            ...
            <motion.section id="projects" variants={item} className="mt-[120px] space-y-10">
              <div className="flex items-center gap-3">
                ...
                <ScrollReveal baseOpacity={0} blurStrength={4} containerClassName="font-serif text-[30px] md:text-[30px] leading-[1.1] font-medium tracking-tight text-text-primary ios-heading">Projects & Events</ScrollReveal>
              </div>
              
              <div className="mt-10">
                <div className="flex flex-wrap gap-4 mb-10">
                  <button
                    onClick={() => setActiveProjectTab('group')}
                    className={`px-6 py-3 rounded-full font-bold uppercase tracking-widest text-xs transition-all ${
                      activeProjectTab === 'group'
                        ? 'bg-accent text-white shadow-lg shadow-accent/20'
                        : 'bg-white/50 text-text-muted hover:bg-white/80 border border-border/40'
                    }`}
                  >
                    Group Projects
                  </button>
                  <button
                    onClick={() => setActiveProjectTab('solo')}
                    className={`px-6 py-3 rounded-full font-bold uppercase tracking-widest text-xs transition-all ${
                      activeProjectTab === 'solo'
                        ? 'bg-accent text-white shadow-lg shadow-accent/20'
                        : 'bg-white/50 text-text-muted hover:bg-white/80 border border-border/40'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setActiveProjectTab('events')}
                    className={`px-6 py-3 rounded-full font-bold uppercase tracking-widest text-xs transition-all ${
                      activeProjectTab === 'events'
                        ? 'bg-accent text-white shadow-lg shadow-accent/20'
                        : 'bg-white/50 text-text-muted hover:bg-white/80 border border-border/40'
                    }`}
                  >
                ...
                <AnimatePresence mode="wait">
                  {activeProjectTab === 'group' && (
                    ...
                    >
                      {GROUP_PROJECTS.map((p) => <GlassProjectCard key={p.name} title={p.name} description={p.desc} image={p.image} tech={p.tech} metrics={p.metrics} />)}
                    </motion.div>
                  )}
                  ...
                  {activeProjectTab === 'solo' && (
                    <motion.div
                      ...
                      className="grid gap-8 md:grid-cols-2 lg:grid-cols-3"
                    >
                      {SOLO_PROJECTS.map((p) => <GlassProjectCard key={p.name} title={p.name} description={p.desc} image={p.image} tech={p.tech} metrics={p.metrics} />)}
                    </motion.div>
                  )}
                  ...
                  {activeProjectTab === 'events' && (
                    <motion.div
                      ...
                      className="grid gap-8 md:grid-cols-2 lg:grid-cols-3"
                    >
                      {EVENTS.map((p) => <GlassProjectCard key={p.name} title={p.name} description={p.desc} image={p.image} tech={p.tech} metrics={p.metrics} />)}
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
            ...
            <motion.section id="skills" variants={item} className="mt-[120px] space-y-10 pb-20">
              <div className="flex items-center gap-3">
                ...
                <ScrollReveal baseOpacity={0} blurStrength={4} containerClassName="font-serif text-[30px] md:text-[30px] leading-[1.1] font-medium tracking-tight text-text-primary ios-heading">Skills & Stack</ScrollReveal>
              </div>

              <div style={{ height: '80px', position: 'relative', overflow: 'hidden', margin: '3rem 0' }}>
                <LogoLoop
                  logos={techLogos}
                  speed={80}
                  direction="left"
